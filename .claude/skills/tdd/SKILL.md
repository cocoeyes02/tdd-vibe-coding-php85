...
Red/Greenフェーズのコミットには、**必ず末尾にフェーズタグを付ける**：

- `（Red）` - テストが失敗する状態
- `（Green）` - テストが通る状態

Refactorフェーズはタグ不要（`refactor:` プレフィックスで判別可能）。

### 意味のある単位でコミット

リファクタリング中心の章など、Red→Greenのステップがない場合は、**意味のある単位**でコミットする：

- 例：「Dollarの修正完了」「Francの修正完了」「共通クラスへの移動完了」
- テストが通る状態を維持しながら、論理的なまとまりでコミット
